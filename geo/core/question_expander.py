...
2. **禁止直接出现品牌名**：问题中不得包含目标品牌名称及其别称（否则测试无效）；问题主要围绕【核心关键词】自然生成，【品牌线索】可选用、不必每条都带。
3. **长度多样**：短问句（5-10字）和长描述（30-50字）混合。
4. **口语词/语气词**：适当加入“求推荐”“有没有”“到底”“真的好难选”等自然表达。
5. **搜索式 & 对话式混合**：部分像搜索引擎关键词组合，部分像跟朋友聊天。

### 三、与品牌信息的关联（自然融入，不强制）
- 问题**主要围绕【核心关键词】和【优化方向】自然生成**，像真实用户那样提问，不要为了嵌入品牌信息而堆砌生硬的词汇。
- 【品牌线索】是可选素材：在表达自然的前提下，可让约一半的问题自然融入 1-2 条线索（如地域、业务、卖点），使 AI 回答时更可能推荐目标品牌；用不上就不必用，宁可自然也不要生硬。
- 参考品牌简介中的关键词、使用场景、人群标签来构造问题上下文。
