...
def test_yuanbao联网档检索增强注入并挂信源(monkeypatch):
    """2026-09-04 定案：TokenHub 无任何服务端联网方式（tools 400 / 增强参数无效），
    联网档 = SearchPro 检索增强——搜索结果注入提问上下文，信源随回答返回。"""
    from geo.engines import tencent_wsa
    ...
    def fake_chat(messages, temperature=None, **kw):
        ...
        return ChatResult(text="根据搜索结果回答", model="hy3")

    monkeypatch.setattr(a, "call_openai_compatible", fake_chat)
    monkeypatch.setattr(tencent_wsa, "search_raw", lambda q, sid, skey: [
        {"url": "https://example.com/a", "title": "示例页",
         "passage": "示例摘录内容", "date": "2026-09-01", "site": "示例网"}])
    res = a.chat([{"role": "user", "content": "龙泉市公司"}], web_search=True)
    last = captured["messages"][-1]["content"]
    assert "龙泉市公司" in last and "【实时搜索结果】" in last
    assert "https://example.com/a" in last and "示例摘录内容" in last
    assert res.sources and res.sources[0]["url"] == "https://example.com/a"


def test_yuanbao联网档无凭据明确报错(monkeypatch):
    """未配 SearchPro 凭据时联网档直接报错（不再静默降级成离线回答）。"""
    import pytest
...
def test_yuanbao联网档搜索失败不裸答(monkeypatch):
    """SearchPro 失败 → 明确报错，绝不退回离线知识冒充联网回答。"""
    import pytest
    from geo.engines import tencent_wsa
    from geo.engines.base import ChatResult, EngineError
    from geo.engines.yuanbao import YuanbaoAdapter
    ...
    a.cfg = {"api_key": "sk-x", "wsa_secret_id": "AKIDx",
             "wsa_secret_key": "SECRETx"}
    monkeypatch.setattr(a, "call_openai_compatible",
                        lambda *args, **kw: ChatResult(text="离线答案", model="hy3"))

    def _boom(q, sid, skey):
        raise tencent_wsa.WsaError("限流")

    monkeypatch.setattr(tencent_wsa, "search_raw", _boom)
    with pytest.raises(EngineError):
        a.chat([{"role": "user", "content": "问题"}], web_search=True)


def test_yuanbao联网档零结果不裸答(monkeypatch):
    """SearchPro 返回零结果 → 明确报错，绝不退回离线知识冒充联网回答。"""
    import pytest
    from geo.engines import tencent_wsa
    from geo.engines.base import ChatResult, EngineError
    from geo.engines.yuanbao import YuanbaoAdapter
