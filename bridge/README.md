...
- 宿主每 30 秒向 GEO 心跳（独立守护线程：单任务发布可达 150s，串行执行不会饿跳误显示「离线」）；主循环串行领取待发任务 → 建稿 → 发布 → 轮询 → 回写
- 发布任务带幂等键（GEO 渠道任务 id）：宿主中断后重试时扩展按键复用任务，已发布的直接回写成功不重发，避免平台产生重复文章
- 卡在「分发中」超过 30 分钟的任务，会在分发相关接口被访问时自动回收为「失败」（拿到重试按钮）
- 发布失败的任务在分发页可单独「重试」
- 日志：`data/bridge.log`
